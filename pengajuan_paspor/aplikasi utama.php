...
// Validasi jika koneksi gagal agar langsung ketahuan error-nya
if (!$koneksi) {
...
if (isset($_POST['btn_daftar'])) {
    ...
    $jadwal_datang = $tgl_datang . " 09:00:00"; // Default Jam 09:00 pagi
    ...
    // Dapatkan ID barusan dimasukkan untuk memicu instansiasi di tabel pengurusan otomatis
...
if (isset($_POST['btn_proses_berkas'])) {
    ...
    // Ketentuan: jika KK, KTP, Ijazah/Akte semua ada maka "lengkap", status diterima, keterangan=OK, Pembayaran=355rb
    if ($ktp == 'Ada' && $kk == 'Ada' && $ijazah_akta == 'Ada') {
        ...
        // Nomor antrian dibuat otomatis berurutan bagi yang OK
    ...
    // Penanganan nilai NULL untuk kolom integer di MySQL agar tidak error format string
    ...
    header("Location: index.php?page=pengurusan");
    exit();
}
...
<head>
    ...
    <style>
        ...
        .header { background: #0056b3; color: white; padding: 15px; border-radius: 5px; margin-bottom: 20px; }
        ...
        .navbar a.active { background: #0056b3; color: white; }
...
<div class="header">
    ...
    <p><strong>Programmer:</strong> [Nama Mahasiswa]</p> 
</div>
    ...
    <?php if ($page == 'daftar'): ?>
        ...
        <table>
            ...
            <tbody>
                ...
                if (!$query) {
                    die("<tr><td colspan='4' style='color:red;'><b>Query Error:</b> " . mysqli_error($koneksi) . "</td></tr>");
                }
    ...
    <?php elseif ($page == 'pengurusan'): ?>
        ...
        <table>
            ...
            <tbody>
                ...
                if (!$query) {
                    die("<tr><td colspan='7' style='color:red;'><b>Query Error:</b> " . mysqli_error($koneksi) . "</td></tr>");
                }
                while ($row = mysqli_fetch_assoc($query)):
